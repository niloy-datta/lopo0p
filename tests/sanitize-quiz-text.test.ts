...
describe("Quiz Sanitization Pipeline", () => {
  ...
  describe("OCR and Corruption cleaning", () => {
    ...
    it("fixes board OCR corruption in question stems", () => {
      expect(sanitizeQuestionText("উদ্ভিদের বৃদ্ধি রেকর্ড করার যন্ত্রের নামÑ")).toBe(
        "উদ্ভিদের বৃদ্ধি রেকর্ড করার যন্ত্রের নাম?",
      );
      expect(sanitizeQuestionText("২০০ঠ হলে সT্চিত")).toContain("২০০ V");
      expect(sanitizeQuestionText("বা®ক্সায়ন সর্বাধিক হবÑে")).toContain("বাষ্পায়ন");
    });

    it("clears image placeholder options like ঘ/A", () => {
      expect(sanitizeOptionText("ঘ/A")).toBe("");
      expect(sanitizeOptionText("K")).toBe("");
    });
  });
  ...
  describe("Placeholder detection and handling", () => {
    ...
    it("strips board practice variant suffix from question stems", () => {
      const input =
        "একটি ম্যাট্রিক্স একই সাথে symmetric ও skew-symmetric হলে সেটি কী? — বোর্ড অনুশীলন ভ্যারিয়েন্ট 05-25 হিসেবে সঠিক উত্তর নির্ণয় করো।";
      expect(sanitizeQuestionText(input)).toBe(
        "একটি ম্যাট্রিক্স একই সাথে symmetric ও skew-symmetric হলে সেটি কী?",
      );
    });
  });

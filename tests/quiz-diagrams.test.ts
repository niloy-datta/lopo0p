...
describe("quiz-diagrams", () => {
  ...
  it("trusts only explicit diagram slugs, not premium placeholders", () => {
    expect(isTrustedStoredDiagram("/images/quiz/ssc-charge-spheres.svg")).toBe(true);
    expect(isTrustedStoredDiagram("/images/quiz/geo-circle-center-o.svg")).toBe(false);
    expect(isTrustedStoredDiagram("/images/quiz/generated/foo-bar.svg")).toBe(false);
    expect(isTrustedStoredDiagram("/images/quiz/premium/foo-bar.svg")).toBe(false);
    expect(isTrustedStoredDiagram("/images/quiz/premium/physics-ssc-science-physics-board-questions-year-wise-2024-dhaka-ts-1.svg")).toBe(false);
    expect(isTrustedStoredDiagram("/images/quiz/premium/options/foo-bar_option_ka.svg")).toBe(false);

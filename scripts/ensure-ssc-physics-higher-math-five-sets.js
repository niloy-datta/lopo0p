...
const { isLowQualitySet, isGarbledBijoyText, isBrokenPhysicsImport } = require("./lib/ssc-set-quality");
const { normalizeStemForDedup } = require("./lib/ssc-biology-stem-utils");
const { buildPhysicsSet } = require("./lib/ssc-physics-premium-bank");

const ROOT = path.resolve(__dirname, "..");
const BACKUP_QUESTIONS_DIR = path.join(ROOT, "data", "backups", "svg-fix-2026-06-28", "public", "questions");
const TARGET_SETS = 5;
const TARGET_Q = 25;
...
function readPublicSetWithBackup(filePath, subjectSlug, subjectDirName) {
  const live = readPublicSet(filePath, subjectSlug);
  if (live.length >= TARGET_Q - 2) return live;
  const backupPath = path.join(BACKUP_QUESTIONS_DIR, subjectDirName, path.basename(filePath));
  if (!fs.existsSync(backupPath)) return live;
  const backup = readPublicSet(backupPath, subjectSlug);
...
function loadChapterSetCandidates(subjectSlug, chapterNo, mega) {
  ...
  if (fs.existsSync(questionsDir)) {
    for (const f of fs.readdirSync(questionsDir)) {
      ...
      const questions = readPublicSetWithBackup(path.join(questionsDir, f), subjectSlug, subjectSlug);
...
function isUsableQuestion(q, subjectSlug) {
  ...
  if (subjectSlug === "physics" && isBrokenPhysicsImport(text)) return false;
  if (!q.options || q.options.length < 4) return false;
  return true;
}

function finalizeSetQuestions(subjectSlug, chapterNo, chapterName, setNo, questions) {
  const seen = new Set();
  const pool = [];
  for (const q of questions) {
    if (!q || !isUsableQuestion(q, subjectSlug)) continue;
  ...
  return generateUniqueSet(subjectSlug, chapterNo, chapterName, setNo, pool).slice(0, TARGET_Q);
}

function buildSetQuestions(subjectSlug, chapterNo, chapterName, setNo, candidates, pool) {
  const bestForSet = candidates.find((c) => c.setNum === setNo);
  const source =
    bestForSet?.questions?.length >= TARGET_Q - 5
      ? bestForSet.questions
      : candidates.sort((a, b) => b.questions.length - a.questions.length)[0]?.questions ?? [];

  if (subjectSlug === "physics") {
    return buildPhysicsSet(chapterNo, chapterName, setNo, source);
  }
  return finalizeSetQuestions(subjectSlug, chapterNo, chapterName, setNo, source);
}

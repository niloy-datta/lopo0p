...
function questionIdFor(subjectSlug, setId, questionNumber) {
  return `hsc-${subjectSlug}-tier-a-hot-model-test-${pad2(setId)}-q${pad2(questionNumber)}`;
}
...
function importHyperMega(sourcePath, subjectSlug = "physics-2nd-paper", level = "hsc") {
  ...
  for (const set of sets) {
    ...
    const meta = {
      displayTitle: `মডেল টেস্ট · সেট ${toBanglaSetNo(setId)}`,
      name: `Board Standard Model Test ${pad2(setId)}`,
      tags: ["board-standard", "high-priority", "paper-wise"],

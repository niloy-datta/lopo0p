...
const TRUSTED_DIAGRAM_SLUGS = new Set([
  ...
  "geo-coord-triangle",
  "geo-triangle-right",
  "geo-triangle-altitudes",
  "geo-circle-equation",
  "geo-cylinder-generator",
  "geo-trapezoid",
  "circuit-series",
  "hm-parabola-y-x2",
...
function isTrustedDiagram(image) {
  if (!image || typeof image !== "string") return false;
  if (!image.startsWith("/images/quiz/")) return false;

  const rel = image.replace(/^\/images\/quiz\//, "");
  const slug = rel.replace(/\.svg$/i, "");
  const fileSlug = slug.split("/").pop();
  if (TRUSTED_DIAGRAM_SLUGS.has(slug) || TRUSTED_DIAGRAM_SLUGS.has(fileSlug)) {
    return true;
  }
  if (GRAPH_OPTION_FAMILIES.some((family) => fileSlug.startsWith(`${family}-`))) {
    return true;
  }

  const abs = path.join(ROOT, "public", "images", "quiz", rel);
  if (!fs.existsSync(abs)) return false;

  // Question-specific generated/premium diagrams are valid when the asset exists.
  if (/^(generated|premium)\//.test(rel)) return true;

  // Root-level question-specific SVGs are also valid when they are tied to a
  // concrete question id rather than a generic placeholder name.
  return /^(hsc|ssc)-[a-z0-9-]+(?:q|set|model-test|chapter)/i.test(fileSlug);
}

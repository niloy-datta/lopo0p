...
const {
  ...
  generateBiologyChapterSets,
  loadQualityPool,
} = require("./lib/generate-ssc-biology-mcqs");
...
function ensureChapter(chapterNo) {
  ...
  const poolStats = loadQualityPool().stats ?? {};
  const chKey = pad2(chapterNo);
  console.log(`  pool: ${poolStats[chKey] ?? 0} harvested + premium`);

  const allSets = generateBiologyChapterSets(chapterNo);
  for (let setNo = 1; setNo <= TARGET_SETS; setNo++) {
    const questions = allSets[setNo - 1] ?? [];

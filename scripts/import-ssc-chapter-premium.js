...
function isPlaceholderPremiumQuestion(q, subjectSlug) {
  ...
  const optionTexts = normalizeOptionsArray(q.options).map((option) => option.text);
  ...
  if (isJunkQuestionText(questionText, subjectSlug)) return true;
  return /^.+\sQ\d+$/i.test(questionText);
}
...
function importPremium(subjectSlug, sourcePath) {
  ...
  for (const ch of chapters) {
    ...
    for (const set of ch.sets ?? []) {
      ...
      const usableQuestions = set.questions.filter((q) => !isPlaceholderPremiumQuestion(q, subjectSlug));
      if (!usableQuestions.length) {
        console.warn(`   ⚠️  Skip placeholder-only set ${slug}`);
        continue;
      }

      const converted = usableQuestions.map((q) =>
    ...
    const chQ = (ch.sets ?? []).reduce(
      (a, s) => a + (s.questions ?? []).filter((q) => !isPlaceholderPremiumQuestion(q, subjectSlug)).length,
      0,
    );
